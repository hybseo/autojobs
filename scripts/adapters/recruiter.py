...
구버전(appsite) 사이트에 대하여
-------------------------------
리크루터에는 두 가지 화면이 있습니다.

  새 버전  {code}.recruiter.co.kr/career/home       → 이 어댑터가 쓰는 API 가 통함
  구버전   {code}.recruiter.co.kr/appsite/company/  → 같은 API 가 HTTP 400

구버전은 서버가 HTML 을 완성해 보냅니다. 별도 API 호출이 없습니다.
2026-09-04 가온전선에서 확인한 구조는 이렇습니다.

  <li>
    <a href="/app/jobnotice/view?systemKindCode=...&jobnoticeSn=...">
      <span class="state state-3">마감</span>
      <span class="date">2026/07/23 ~ 2026/08/03</span>
      <span class="title">'26년 3분기 인턴사원 수시채용</span>
    </a>
  </li>

  state-3 이 마감입니다. 접수중은 다른 번호를 쓸 텐데 확인하지 못했습니다.
  가온전선의 공고가 그 시점에 전부 마감이었기 때문입니다.

지금 이 구조를 지원하지 않는 이유
  대상이 가온전선 하나뿐이고 접수중 공고가 0건이라, 만들어도 검증할
  실제 데이터가 없습니다. 접수중 공고가 있는 구버전 사이트를 만나면
  그때 만드세요. 위 구조 그대로면 30분이면 됩니다.

구버전이 아닌데도 400 이 나는 경우
  효성·고려아연은 appsite 경로에도 목록이 없습니다. 회사가 자체 화면을
  붙인 경우로, 구버전 어댑터로도 해결되지 않습니다.
  고려아연은 "게시된 디자인을 찾을 수 없습니다" 라는 응답을 줍니다.
"""
import json
import time
import html
